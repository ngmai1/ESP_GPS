...
class FlaskThread(QThread):
    ...
    @flask_app.route('/esp32', methods=['POST'])
    def esp32():
        # Get the posted data
        ...
        FlaskThread.update_log.emit(latitude,longitude,angle,rs_angle,utc,gps_date)
    ...
    def run(self):
        flask_app.run(host='192.168.8.250', port=3000,debug=True, use_reloader=False)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                my_map = folium.Map(location=coordinates[0], zoom_start=18)
...
class MyWindow(QMainWindow):
    ...
    def append_log_flask(self, lat,lon,angle,rs_angle,utc,gps_date):
        self.ui.lbl_latitude.setText(lat)
        self.ui.lbl_longitude.setText(lon)
        self.ui.lbl_angle.setText(angle)
        self.ui.lbl_rs_angle.setText(rs_angle)
